            ...
            if is_short_url:
                if url_expanded_successfully:
                    ...
                    try:
                        ...
                        for safe_domain_lower, (safe_domain, description) in normalized_safe_domains.items():
                            if domain.endswith('.' + safe_domain_lower):
                                ...
                                # 特殊處理：cht.tw 短網址 - 展開後連到安全網站的子網域
                                elif original_url and 'cht.tw' in original_url.lower():
                                    return {
                                        "success": True,
                                        "message": "分析完成",
                                        "result": {
                                            "risk_level": "低風險",
                                            "fraud_type": "中華電信官方短網址連到安全網站",
                                            "explanation": f"✅ 這是中華電信官方短網址，連到安全的網站。",
                                            "suggestions": "✅ 這個中華電信短網址是安全的，可以放心使用\n🔍 建議確認傳送者身份，確保不是陌生人\n⚠️ 如果內容涉及投資、賺錢、購物，要特別小心\n🛡️ 遇到要求轉帳或提供個人資料時，務必多方查證\n💡 養成良好習慣：即使是官方平台也要保持警覺",
                                            "is_emerging": False,
                                            "display_name": display_name,
                                            "original_url": original_url,
                                            "expanded_url": expanded_url,
                                        ...
                                        "raw_result": f"中華電信官方短網址連到安全網站：{description}"
                                    }
                    ...
                    if original_url and 'lin.ee' in original_url.lower():
                        ...
                        # 修改分析訊息，包含短網址和展開後的資訊
                        analysis_message = f"這是一個中華電信官方短網址 {original_url}，展開後連到 {expanded_url}。請分析展開後的網站內容是否為詐騙，特別注意：1. 是否為投資詐騙 2. 是否為購物詐騙 3. 是否為假交友詐騙 4. 是否要求提供個人資料或金錢 5. 網站內容是否可疑。由於詐騙集團可能假冒中華電信官方來傳送詐騙連結，請特別仔細分析。"
                    else:
                        # 一般短網址的分析
                    ...
                    # 特殊處理：cht.tw 無法展開的情況
                        ...
                        if suspicious_content:
                            # 如果訊息內容可疑，進行完整分析而不是直接返回
                            logger.warning(f"cht.tw 無法展開且訊息內容可疑，進行完整分析")
                            analysis_message = f"這個訊息包含一個無法展開的中華電信官方短網址 {original_url}，同時訊息內容可能涉及詐騙。請仔細分析整個訊息內容是否為詐騙，特別注意投資詐騙、購物詐騙等常見手法。訊息內容：{user_message}"
                            # 繼續進行 OpenAI 分析，不直接返回
                        else:
                            # 如果訊息內容看起來正常，返回中風險警告
                            return {
                                ...
                                "result": {
                                    "risk_level": "中風險",
                                    "fraud_type": "中華電信官方短網址無法展開",
                                    "explanation": f"⚠️ 這是一個中華電信官方短網址，但我們無法展開查看真正的目的地。\n\n🔍 短網址：{original_url}\n\n可能原因：\n• 連結已失效或過期\n• 中華電信服務暫時無法訪問\n• 網站維護中\n• 可能是惡意連結被封鎖\n\n💡 雖然是中華電信官方服務，但詐騙集團也可能假冒官方。無法展開的連結需要特別小心。",
                                    "suggestions": "🔍 詢問傳送者這個連結的具體內容和目的\n⚠️ 確認傳送者身份是否可信\n🛡️ 如果不確定來源或內容，建議不要點擊\n📞 如有疑慮可撥打165反詐騙專線諮詢\n💡 記住：即使是官方平台，也可能被不肖人士利用",
                                    "is_emerging": False,
                                    "display_name": display_name,
                                    "original_url": original_url,
                                ...
                                "raw_result": f"中華電信官方短網址無法展開，需要謹慎確認：{original_url}"
                            }
                    else:
                        # 一般短網址無法展開，設定分析訊息但不直接返回
                        analysis_message = user_message
                    
                    # 如果不是 lin.ee 或 cht.tw 或內容不可疑，對於一般短網址無法展開直接返回高風險
                    if not (original_url and ('lin.ee' in original_url.lower() or 'cht.tw' in original_url.lower())):
                        return {
                            "success": True,
                            "message": "分析完成",
                            "result": {
    ...
    if found_urls:
        for url in found_urls:
            ...
            short_url_domains = [
                ...
                'reurl.cc', 'pse.is', 'lihi.io', 'lin.ee', 'cht.tw'
            ]
