...
說明：[用80-100字簡單易懂的話解釋，像鄰居朋友在聊天的語氣，避免技術術語，重點說明為什麼要小心]
建議：[用emoji符號（🚫🔍🌐🛡️💡⚠️等）開頭，給出3個以內的簡單明確防範建議]

重要提醒：
1. 說明部分要控制在80-100字內，用日常生活的話來解釋
2. 避免使用「URL」、「域名」、「伺服器」等技術詞彙，改用「網址」、「網站」等
3. 重點說明「為什麼要小心」而不是技術細節
4. 建議要具體可行，不要超過3個
5. 語氣要像關心的鄰居朋友，不要太正式
6. 請用中文回答
"""
